            ...
            if (is_file($file) === true) {
                if (($lines = file($file, FILE_SKIP_EMPTY_LINES)) !== false) {
                    ...
                    foreach ($lines as $templine) {
                        ...
                        $temp_data['birth']         = date('Y-m-d', strtotime(str_replace('/', '-', $data[10])));   // Fecha de nacimiento
                        ...
                        $temp_data['phone_1']       = str_replace($especiales, $reemplazos, $data[16]);                        // Telefono domicilio
                        $temp_data['phone_2']       = str_replace($especiales, $reemplazos, $data[17]);                        // Telefono celular
                        $temp_data['email'] = '';                           // Email
                        $temp_data['phone_office']  = str_replace($especiales, $reemplazos, $data[18]);           // Telefono oficina
                        $occ_code                   = '';                                     // Ocupacion
...
if(isset($_GET['idCl'])){
    ...
    if (($rsUp = $link->query($sqlUp)) !== false) {
        if($rsUp->num_rows === 1){
...
?>
        ...
        <div class="content-input">
            <input type="text" id="dc-locality-<?=$k?>" name="dc-locality-<?=$k?>" 
                autocomplete="off" value="<?=$data['locality'];?>" class="required text-2 fbin">
        </div><br>
    </div><!--
    --><div class="form-col">
...
<?php if (!$web_service && $nCl < $max_item && !$swCl): ?>
    <script type="text/javascript">
