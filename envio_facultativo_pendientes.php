...
WHERE t0.facultativo = 1 and t0.emitir = 0 and t0.anulado = 0 and t0.id_emision not in (Select id_emision from s_de_facultativo)
			...
			$shtml.='<tr style="background:#D3DCE3;">
				...
				while($resutadodes=mysql_fetch_array($consultades)){
				 ...
				 $shtml.='<tr>
							  ...
							  if($resutadodes['detalle_p'] != ''){
								  $detp = json_decode($resutadodes['detalle_p'], true);								  
								  
								  $shtml.='<td align="center">'.$detp["estado"].'</td>
								  <td align="center">'.$detp["observacion"].'</td>';
							  }else{
								  $shtml.='<td align="center">'.$resutadodes['estado'].'</td>
							  <td align="center">'.$resutadodes['observacion'].'</td>';
							  }
							  $shtml.='<td align="center">'.$resutadodes['agencia'].'</td>
