...
class ReportsPropDE{
   ...
   public function getChange(){
		$this->cn=new SibasDB();
		$this->consulta="select valor_boliviano from s_tipo_cambio";
		$this->result=$this->cn->query($this->consulta,MYSQLI_STORE_RESULT);
		if($this->result->num_rows>0){
		$this->fila = $this->result->fetch_array(MYSQLI_ASSOC);
		$cambio=$this->fila['valor_boliviano'];
		return $cambio;
			}
		}
	...
	private function set_query_de_report(){
		...
		$this->sql = "select
			...
			(case 
	            when sde.moneda='USD' and sde.monto_solicitado <=15000 then 'FREECOVER' 
			    when sde.moneda='USD' and sde.monto_solicitado between 15000 and 50000 then 'CREDISEGURO 1'
			    when sde.moneda='USD' and sde.monto_solicitado between 50000 and 150000 then 'CREDISEGURO 2'
			    when sde.moneda='USD' and sde.monto_solicitado between 150000 and 250000 then 'CREDISEGURO 3'
				
				when sde.moneda='BS' and sde.monto_solicitado <=15000*".$change." then 'FREECOVER' 
			    when sde.moneda='BS' and sde.monto_solicitado between 15000*".$change." and 50000*".$change." then 'CREDISEGURO 1'
			    when sde.moneda='BS' and sde.monto_solicitado between 50000*".$change." and 150000*".$change." then 'CREDISEGURO 2'
			    when sde.moneda='BS' and sde.monto_solicitado between 150000*".$change." and 250000*".$change." then 'CREDISEGURO 3'
...
<table class="result-list" id="result-de">
	<thead>
    	<tr>
			...
			<td><?=htmlentities('Cúmulo desembolsos anteriores (a)', ENT_QUOTES, 'UTF-8');?></td>
            <td>Monto solicitado (b)</td>
            <td><?=htmlentities('Cúmulo total (a+b)', ENT_QUOTES, 'UTF-8');?></td>
...
?>
		<tr style=" <?=$bg;?> " class="row" rel="0"
            ...
            <td><?php
			//cúmulo de desembolsos
            	if( $bc==1 ){
					if($this->row['r_moneda']==='Dolares'){
						$change=$this->getChange();
						$this->rowcl['d_saldo']=$this->rowcl['d_saldo']/$change;
					//echo number_format($this->rowcl['d_saldo'],2,'.',',');
				      echo number_format($this->rowcl['d_saldo'],2,'.',',')." USD";
				    }else
				      echo number_format($this->rowcl['d_saldo'],2,'.',',')." Bs"; 
				}else{
					if($this->row['r_moneda']=='Dolares'){
						$change=$this->getChange();
						$this->row['d_saldo']=$this->row['d_saldo']/$change;
					//echo number_format($this->rowcl['d_saldo'],2,'.',',');
				      echo number_format($this->row['d_saldo'],2,'.',',')." USD";
					}else{
            		echo number_format($this->row['d_saldo'],2,'.',',')." Bs";
					}
				}
            ?></td>
            <td><?php
			// monto solicitado en base a tipo de cobertura
            	if( $bc==1 ){
					if($this->row['r_moneda']==='Dolares'){
                    echo number_format($this->rowcl['d_monto_banca_comunal'],2,'.',',')." USD";			
					}else{
					echo number_format($this->rowcl['d_monto_banca_comunal'],2,'.',',')." Bs";
						}
				}else{
					if($this->row['r_moneda']==='Dolares'){
		              echo number_format($this->row['r_monto_solicitado'],2,'.',',')." USD";		
				         }else{
            		   echo number_format($this->row['r_monto_solicitado'],2,'.',',')." Bs";
		             }
				}
            ?></td>
            <td><?php
			//cúmulo total de ambos montos
            	if( $bc==1 ){
					if($this->row['r_moneda']==='Dolares'){
						$change=$this->getChange();
						$this->rowcl['d_cumulo']=$this->rowcl['d_cumulo']/$change;
            		echo number_format($this->rowcl['d_cumulo'],2,'.',',')." USD";
					}else{
					echo number_format($this->rowcl['d_cumulo'],2,'.',',')." Bs";
						}
				}
            	else{
            		if($this->row['r_moneda']==='Dolares'){
						$this->row['d_cumulo']=$this->row['d_cumulo']/7;
            		echo number_format($this->row['d_cumulo'],2,'.',',')." USD";
					}else{
					echo number_format($this->row['d_cumulo'],2,'.',',')." Bs";
						}
				}
            ?></td>
			...
			<td>-</td>
			<td><?php if($bc==1){
				$monto=$this->rowcl['d_monto_banca_comunal'];
				$change=$this->getChange();
				if($this->row['r_moneda']=='Dolares'){
					switch($monto){
						case($monto<=15000):
						echo 'FREECOVER';
						break;
						case($monto>15000 && $monto<=50000):
						echo 'CREDISEGURO 1';
						break;
						case($monto>50000 && $monto<=150000):
						echo 'CREDISEGURO 2';
						break;
						case($monto>150000 && $monto<=250000):
						 echo 'CREDISEGURO 3';
						break;
						default:
						echo "SUPERA EL REQUERIMIENTO";
					}	
					}else if($this->row['r_moneda']=='Bolivianos'){
					 switch($monto){
						 case($monto<=(15000*$change)):
						echo 'FREECOVER';
						break;
						case($monto>(15000*$change) && $monto<=(50000*$change)):
						echo 'CREDISEGURO 1';
						break;
						case($monto>(50000*$change) && $monto<=(150000*$change)):
						echo 'CREDISEGURO 2';
						break;
						case($monto>(150000*$change) && $monto<=(250000*$change)):
						 echo 'CREDISEGURO 3';
						break;
						default:
						echo "SUPERA EL REQUERIMIENTO";
